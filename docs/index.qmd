...
Identifying the factors that influence to organism growth are crucial to understanding how conditions in the wild affect ecological outcomes over generations. The aim of this study is to understand conditions that influence the larval growth rates and adult weights of *Pieris rapae*. In the data we used, mothers reared on one of two different plants, *Barbarea* and *Berteroa*, and oviposited on one of those same two plants, after which responses in offspring development were recorded to provide insight into the above issues.
...
Two two-way ANOVA analyses were performed to investivate the relationship between the two factors (maternal host and larval host). We fitted a linear model to each response variable and both factors, producing one model for the effect of the factors on growth rate and one model for the effect of the factors on adult weight.
